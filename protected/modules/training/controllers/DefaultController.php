...
class DefaultController extends Controller
{
	...
	public function actionLogout()
	{
		...
		Yii::app()->getModule('training')->user->logout(false);
		Yii::app()->user->setFlash('success', 'Logout Successful.');
		$this->redirect(Yii::app()->getModule('training')->user->loginUrl);
	}
	...
	public function actionListChapters($rep_id, $region_id)
	{
		...
		if($chapters) {
			foreach($chapters as $key => $chap) {
				$results.='<tr>';
					$results.='<td>'.($key+1).'</td>';
					$results.='<td><a href="'.Yii::app()->createUrl('training/default/viewreport?rep_id='.$rep_id.'&chapter_id='.$chap->chapter_id).'" target="_blank">'.Chapter::model()->getChapter($chap->chapter_id).'</a></td>';
				$results.='</tr>';
			}
		} else {
