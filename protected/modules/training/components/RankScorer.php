...
class RankScorer
{
	...
	*/
	...
	public function __construct($area = null, $count = null)
	{	
		ScoresStorage::setup();  
	...
	public function scoreCollect($reports)
	{
		...
		foreach($reports as $report) {
			...
			foreach($etraining_scores as $key1=>$score) {
				if (strpos($key1, $report->rep_id.':') !== false) {
	...
	{	
		...
		foreach($chapter_scores as $group_name=>$group_scoring) {
			...
			$group_percentage = @($group_scoring['total_score']  /  $group_scoring['perfect_score'] ) * 100;
