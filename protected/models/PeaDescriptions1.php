...
class PeaDescriptions extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('rep_id, sub_id, description, goal_point, color, goal, details', 'required'),
			array('sub_id, goal_point, criteria_point, max, range', 'numerical', 'integerOnly'=>true),
			...
			array('rep_id, sub_id, description, goal_point, criteria_point, max, qty, range, color, goal, criteria, details, remarks', 'safe', 'on'=>'search'),
		);
	}
	...
	public static function computeRaw($sub_id, $chapter_id)
	{
		$raw_points = Yii::app()->db->createCommand()
		    ...
		    ->where('d.sub_id = ' . $sub_id. ' AND d.color = "BLACK" AND s.chapter_id = '.$chapter_id)
		    ->queryRow();
